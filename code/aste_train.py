...
class ASTE(pl.LightningModule):
    def __init__(self, hparams, data_module):
        ...
        self.config.current_epoch = self.current_epoch
    ...
    def training_step(self, batch, batch_idx):
        ...
        outputs = self(**batch)
        loss = outputs['table_loss_S'] + outputs['table_loss_E'] + outputs['pair_loss'] + outputs['CL_senti'] + outputs['CL_edge']
        return loss
    ...
    def setup(self, stage):
        if stage == 'fit':
            ...
            # self.train_loader = self.trainer._data_connector._train_dataloader_source.dataloader()
            ngpus = 1
...
def main():
    ...
    trainer = pl.Trainer.from_argparse_args(args, **kwargs, gpus=[0])
    trainer.fit(model, datamodule=data_module)
    model.load_model()
    trainer.test(model, datamodule=data_module)

if __name__ == '__main__':
    if os.path.exists("..\code/0.txt"):
        os.remove("..\code/0.txt")
    for i in range(1):
        main()
    avg_f1, avg_p, avg_r = avg_result()
    f_out = open('../code/avg_log.txt', 'w+', encoding='utf-8')
    f_out.write('\tF1:{:.5f}\tP:{:.5f}\tR:{:.5f}\n'.format(avg_f1, avg_p, avg_r))
    print("F1:")
    print(avg_f1)
    f_out.close()